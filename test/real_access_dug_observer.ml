...
let artifact_for_global ~source before pre =
...
let artifact_for_module source =
  ...
  artifact_for_global ~source before pre


let modules = ref []
let out_dir = ref ""
let add_module path = modules := !modules @ [path]
let usage = "real_access_dug_observer --module <file.c>... --out <dir>"
...
let () =
  Arg.parse
    [
      "--module", Arg.String add_module, "C source module to run through frozen real Access+DUG observer";
