...
|> ApiMap.add "fcntl" {arg_typs = [Skip; Skip; StructPtr]; ret_typ = int_v}
...
|> ApiMap.add "version_etc_va" {arg_typs = [Skip; Skip; Skip; Skip; Skip]; ret_typ = ones}
...
|> ApiMap.add "fclose" {arg_typs = [Skip]; ret_typ = int_v}
...
|> ApiMap.add "textdomain" {arg_typs = [Skip]; ret_typ = int_arr}
