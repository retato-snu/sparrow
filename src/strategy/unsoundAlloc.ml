...
let rec ptr_of_exp op exp =
  match exp with
  | BinOp (op0, CastE (_, _, e1), e2, t)
  | BinOp (op0, e1, CastE (_, _, e2), t) when op = op0 ->
...
let is_alloc_ptr_instr instr p =
  ...
  | (Call (Some (Var v, NoOffset), Lval (Var f, NoOffset), _, _, _))::_ ->
...
let rec is_alloc_ptr stmt p =
  ...
  | [pred] ->
    (match pred.skind with
     | Instr [Set ((Var v, NoOffset), Lval (Var tmp, NoOffset), _, _)]
     | Instr [Set ((Var v, NoOffset), CastE (_, _, Lval (Var tmp, NoOffset)), _, _)]
...
let transform file =
  ...
  ignore(Sparrow_cil.visitCilFile vis file);
