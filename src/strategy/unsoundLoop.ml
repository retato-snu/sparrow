...
= fun global ->
  if !Options.bugfinder < 1 then BatSet.empty
    ...
    let classifier = Py.Callable.to_function load [| Py.String.of_string (sparrow_data_path ^ "/harmless_loop_clf") |] in
    ...
    in
    if !Options.debug then
      (prerr_endline "== harmless loops ==";
      BatSet.iter prerr_endline set);
    set
